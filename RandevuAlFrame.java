...
public class RandevuAlFrame implements Initializable {
    ...
    private void setBirimlerItems(){
        ...
        tree.root = new TreeNode<>("Hastane");
        tree.root.right = new TreeNode<>("Aile Sağlığı Merkezi");

        LinkedList<String> list = new LinkedList<>();
        tree.traverseInOrder(tree.root, list);

        for(int i=0 ; i<list.size() ; i++)
        {
            birimler.getItems().add(list.get(i));
        }
    ...
    private void setOdalarItems(){
        ...
        queue.enqueue("Aşı Odası 1");
        queue.enqueue("Aşı Odası 2");
        queue.enqueue("Aşı Odası 3");
        queue.enqueue("Aşı Odası 4");
        queue.enqueue("Aşı Odası 5");

        String[] odalarStringArray = new String[queue.A.size()];
        for(int i=0 ; i<odalarStringArray.length ; i++)
        {
            odalarStringArray[i] = queue.getHead();
            queue.dequeue();
        }
